...
factGibbsMod <- function(X, k, U0, r0, sigma0, Ut, No_Iter, burnin, thin) {
  ...
  for (iter in 1:(No_Iter + burnin + 1)) {
    ...
    V1 = ((sigma0^2)/(r_cur^2 + sigma0^2))*diag(k)
    G_temp <- MASS::mvrnorm(n, rep(0, k), V1)
    datorth = X %*% U_cur
    G_cur = G_temp + datorth*(r_cur/(r_cur^2 + sigma0^2))  
    ...
    # Update of r
    V2 = sigma0^2/facttrace
    mu2 = (factdata - sigma0^2)/facttrace
    r_cur <- truncnorm::rtruncnorm(1, mean = mu2, sd = V2,  a = sigma0)
